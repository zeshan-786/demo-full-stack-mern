...
module.exports = {
  ...
  show: async (req, res) => {
    try {
      ...
      const Clinic = await ClinicModel.findOne({ _id: id }).select(
        "-salt -hash"
      );
      if (!Clinic) {
      ...
      return res.status(500).json({
        message:
          error && error.message ? error.message : "Error when getting Clinic.",
      });
    }
  },
  ...
  update: async (req, res) => {
    try {
      ...
      if (req.body.email) {
        const dupCheck = await ClinicModel.findOne({ email: req.body.email });
