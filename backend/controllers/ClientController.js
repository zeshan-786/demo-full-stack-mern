...
module.exports = {
  ...
  list: async (req, res) => {
    try {
      const users = await ClientModel.find(req.query.where, req.query.fields, req.query.sort).select('-salt -hash')
      return res.json(users)
    } catch (error) {
      return res.status(500).json({
        message: error && error.message ? error.message : 'Error when getting Users.'
      })
    }
  },
  ...
  show: async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: error && error.message ? error.message : 'Error when getting User.'
      });
    }
  },
