...
module.exports = {
  ...
  show: async (req, res) => {
    try {
      const id =
        req.query.id || !["Admin", "Clinic"].includes(req.user.__userType)
          ? req.user._id
          : null;
      ...
      return res.status(500).json({
        message:
          error && error.message ? error.message : "Error when getting Doctor.",
      });
    }
  },
  ...
  update: async (req, res) => {
    try {
      ...
      if (req.body.email) {
        const dupCheck = await DoctorModel.findOne({ email: req.body.email });
